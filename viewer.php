...
for ($i=0; $i<count($result_arr); $i++){
    ...
    for ($j=0; $j<count($segment_arr); $j++){
        ...
        echo "<td><input type=\"checkbox\" id=\"no__".$segment_arr[$j]["Gene"]."__".$j."\" onclick=\"checkbox_highlights(this)\"></td>";
            ...
            if ($key != "Position" && $key != "Genotype_with_Description"){
                if(!intval($key)){
                    ...
                    if (preg_match("/missense.variant/i", $value)){
                        ...
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$frameshift_variant_color_code."\">".$value."</td>";
                    } else if (preg_match("/exon.loss/i", $value)){
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$exon_loss_variant_color_code."\">".$value."</td>";
                    } else if (preg_match("/lost/i", $value)){
                        $temp_value_arr = preg_split("/[;, |\n]+/", $value);
                        $temp_value = (count($temp_value_arr) > 2 ? $temp_value_arr[0]."|".$temp_value_arr[2] : $value);
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$lost_color_code."\">".$value."</td>";
                    } else if (preg_match("/gain/i", $value)){
                        $temp_value_arr = preg_split("/[;, |\n]+/", $value);
                        $temp_value = (count($temp_value_arr) > 2 ? $temp_value_arr[0]."|".$temp_value_arr[2] : $value);
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$gain_color_code."\">".$value."</td>";
                    } else if (preg_match("/disruptive/i", $value)){
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$disruptive_color_code."\">".$value."</td>";
                    } else if (preg_match("/splice/i", $value)){
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$splice_color_code."\">".$value."</td>";
                    } else if (preg_match("/ref/i", $value)){
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:".$ref_color_code."\">".$value."</td>";
                    } else{
                        echo "<td id=\"pos__".$segment_arr[$j]["Gene"]."__".$key."__".$j."\" style=\"min-width:80px;background-color:#FFFFFF\">".$value."</td>";
                    }
                }
            }
        }
...
echo '<script type="text/javascript" language="javascript" src="./js/checkbox_highlights.js"></script>';
